...
function load({ userSettings = {}, theme = 'github-light', webModules = 'default' } = {}) {
    ...
    const setThemeFn = arg => {
        calls.setTheme.push({ name: arg && arg.name })
    }
    const win = {}
    if (webModules === 'default') win.webModules = { config: { setTheme: setThemeFn } }
    else if (webModules === 'missing') win.webModules = undefined
    else win.webModules = webModules // caller-supplied shape
